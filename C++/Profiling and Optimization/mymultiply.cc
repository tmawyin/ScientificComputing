...
void multiply(MyMatrix &a, MyMatrix &b, MyMatrix &c)
{
	...
	// getting matrix double pointer
	double** matrixA = a.get_pointer();
	double** matrixB = b.get_pointer();
	double** matrixC = c.get_pointer();
    ...
    for (int i=0; i<n; i++) {     
        for (int j=0; j<k; j++) {  
			...
			// Using pointer-to-pointer to set to zero
			matrixC[i][j] = 0.0;
            for (int j2=0; j2<m; j2++) {
				...
				// Using pointer-to-pointer to speed up the code
				double newvalue = matrixC[i][j] + matrixA[i][j2]*matrixB[j2][j];
                matrixC[i][j] = newvalue;
            }
        }
    }
}
